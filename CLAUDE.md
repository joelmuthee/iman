...
- **IG auto-sync cron (added 2026-06-12, owner ask):** worker cron `0 6 * * *` (09:00 EAT — Iman holds the fleet's first stagger slot) runs `runIgAutoSync` — the manual "Check for new posts" pipeline minus the review step (feed fetch → heuristic+vision+text classify → caption parse for name/category/sizes/price/gender → cover image to KV → prepend, `autoSynced: true` on the bag). Capped at 20 items/run (Workers Paid budget, ~112 of ~1,000 subrequests; the cap now exists to keep IG fetch volume gentle, not for Cloudflare); never re-adds existing `ig_<shortcode>` ids. Manual trigger: authed `POST /api/autosync-run`. Kill switch: KV `autosync` = `{"enabled":false}`. Suspended shops skip. Cover image only — carousel extras via admin edit. **Commercially this is a Shop Records (3k+) feature** (owner directive 2026-06-12); Iman has it live despite being on Basic — flip the KV flag if that changes.
