...
- **IG auto-sync cron — built here, DISABLED for Iman (owner directive 2026-06-12: 3k+ feature, only ThriftLux + Ryker get it; Iman is Basic).** The code ships in the worker (`runIgAutoSync`, cron `0 6 * * *`, manual trigger `POST /api/autosync-run`, cap 20/run on Workers Paid) but KV **`autosync` = `{"enabled":false}`** makes every run no-op (verified: trigger returns `{"ok":false,"skipped":"disabled"}`). The admin banner deliberately does NOT promise automatic updates (honesty rule in CATALOG-STANDARDS). **Upgrade to 3k = delete the KV key (or set enabled:true) + add the "posts add themselves each morning" sentence back to the banner copy.** Pipeline details + stagger convention: CATALOG-STANDARDS → Instagram bulk sync → Auto-sync cron.
