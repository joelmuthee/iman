// Iman High Street — public catalog
...
(async function () {
  ...
  const NEW_DAYS = 7;
  const LOW_STOCK = 3;
  const WISHLIST_KEY = 'iman_wishlist';
  let items = [];
  let settings = {};
  let suspended = false;
  ...
  const ANALYTICS_KEY = 'iman_analytics';
  ...
  function sizeGroup(s) {
    ...
    if (!isNaN(n) && n >= 6 && n <= 18) return 'Dress size (UK)';
    if (!isNaN(n) && n >= 34 && n <= 58) return 'Suit size';
    if (/\d/.test(u)) return 'Other';
    return 'Other';
  }
  ...
  function buildSizePills() {
    ...
    const order = ['Dress size (UK)', 'Clothing (S-XL)', 'Suit size', 'Shoe size (EU)', 'Shoe size (UK)', 'Other'];
    const grouped = {};
  ...
  document.getElementById('wishlistEnquireAll')?.addEventListener('click', e => {
    ...
    const msg = `Hi Iman High Street! I'd like to check availability of these saved items:\n\n${lines.join('\n')}\n\nAre they available?`;
  ...
  function showSuspended() {
    ...
    const css = ('@keyframes imSusFade{from{opacity:0;transform:translateY(8px)}to{opacity:1;transform:none}}'
      + '#suspendedOverlay{position:fixed;inset:0;z-index:99999;background:radial-gradient(ellipse at top,#3d2817 0%,#2a1c0f 65%);color:#e8dcc4;display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center;padding:40px 24px;font-family:Inter,system-ui,-apple-system,sans-serif;animation:imSusFade 0.65s ease both;}'
      ...
      + '#suspendedOverlay .im-offer{font-size:16px;max-width:460px;line-height:1.6;margin:0 0 30px;color:#e3cd8f;}'
      + '#suspendedOverlay .im-offer b{color:#fff;font-weight:700;}'
      + '#suspendedOverlay .im-ig{display:inline-flex;align-items:center;gap:10px;background:#bf9b30;color:#2a1c0f;padding:14px 30px;border-radius:999px;text-decoration:none;font-weight:600;font-size:15px;letter-spacing:0.3px;box-shadow:0 6px 24px rgba(191,155,48,0.3);transition:transform 0.2s ease,box-shadow 0.2s ease,background 0.2s ease;}'
    ...
    o.innerHTML = (
      '<img class="im-logo" src="' + logoUrl + '" alt="' + shopName + '">'
      + '<div class="im-name">' + shopName + '</div>'
      + (tagline ? '<div class="im-tag">' + tagline + '</div>' : '<div style="height:30px"></div>')
      + '<div class="im-rule"></div>'
      + '<h1 class="im-head">This shop is paused</h1>'
      + '<p class="im-body">Not ready for a monthly plan? You don\'t need one.</p>'
      + '<p class="im-offer">Now you can <b>own this shop outright for a one-time Ksh 20,000</b>, no monthly fees. New stock you post on Instagram pulls straight into your shop. Buyers can filter by category and size to find what they want fast, then order on WhatsApp.</p>'
      + '<a class="im-ig" href="' + waLink + '" target="_blank" rel="noopener">' + WA_SVG + ' Bring my shop back</a>'
    );
