...
function checkAuth() {
  if (sessionStorage.getItem('iman_auth') === '1') {
...
async function login() {
  ...
  try {
    ...
    if (j.ok) { sessionStorage.setItem('iman_auth', '1'); checkAuth(); }
    else { loginError.style.display = 'block'; }
  } catch (e) {
    // Network fallback so a CF outage doesn't lock the owner out.
    if (pw === ADMIN_PASSWORD) { sessionStorage.setItem('iman_auth', '1'); checkAuth(); }
    else { loginError.style.display = 'block'; }
  }
}
document.getElementById('logoutBtn').addEventListener('click', () => {
  sessionStorage.removeItem('iman_auth');
  location.reload();
});
...
function buildColorStockGrid(existing) {
  ...
  if (!sizes.length) { grid.innerHTML = '<p style="font-size:12px;color:#999;">Type the sizes above (e.g. M, L, XL), then tap <strong>Build grid</strong>.</p>'; return; }
...
const RESTOCK_ALL_SIZES = ['XS','S','M','L','XL','XXL','3XL','4XL','5XL','6','8','10','12','14','16','18','34','36','38','40','42','44','46','48','50','52','54','56','58','EU35','EU36','EU37','EU38','EU39','EU40','EU41','EU42','EU43','EU44','EU45','EU46'];
...
const EXPENSES_ENABLED = true; // 5k Shop Manager tier — Iman 5k trial 2026-06-18; flip false on downgrade
...
const SALE_ENABLED = true; // 5k trial (Iman, 2026-06-18) — revert to false on downgrade
...
const LOYALTY_ENABLED = true; // 5k Shop Manager tier — Iman 5k trial 2026-06-18; flip false on downgrade
const LOYALTY_SHOP = 'Iman High Street';
...
const INSIGHTS_KEY = 'iman_analytics';
...
function renderIgSyncList() {
  igSyncListEl.innerHTML = igSyncCandidates.map((it, i) => {
    ...
    const catOpts = SHOP_CATEGORIES.map(c => `<option value="${c}" ${c === s.category ? 'selected' : ''}>${c}</option>`).join('');
    return `
...
function posPrintReceipt() {
  ...
  document.getElementById('posReceiptPrint').innerHTML = `
    <div class="rcpt">
      <div class="rcpt-head">Iman High Street</div>
